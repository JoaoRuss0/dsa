pub fn run() {
    ...
    let input = std::fs::read_to_string("input/codyssi/journey_to_atlantis/problem4.txt")
        .unwrap();
    ...
    let get_message_size = | message : String | -> u64 {
        message.chars().map(get_char_size).sum::<u64>()
    };
