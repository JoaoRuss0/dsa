...
pub fn run() {
    ...
    let input = std::fs::read_to_string("input/codyssi/journey_to_atlantis/problem5.txt").unwrap();

    let distance =
        |p1: (i64, i64), p2: (i64, i64)| -> i64 { (p1.0 - p2.0).abs() + (p1.1 - p2.1).abs() };
    ...
    let (&p_c, &d_c) = distances_boat.iter().min_by(|&a, &b| a.1.cmp(b.1)).unwrap();
    let (_, &d_f) = distances_boat.iter().max_by(|&a, &b| a.1.cmp(b.1)).unwrap();

    println!("  │  ├─ Part 1: {}", (d_c - d_f).abs());

    let closest = |point: (i64, i64), points: &Vec<(i64, i64)>| -> ((i64, i64), i64) {
        let points_but_closest: Vec<(i64, i64)> =
            points.iter().filter(|&p| !p.eq(&point)).copied().collect();

        let distances_closest = distances(point, points_but_closest);

        distances_closest
            .iter()
            .min_by(|(p1, d1), (p2, d2)| d1.cmp(d2).then(p1.0.cmp(&p2.0)).then(p1.1.cmp(&p2.1)))
            .map(|(&(x, y), &d)| ((x, y), d))
            .unwrap()
    };
    ...
    while to_visit.len() > 1 {
        ...
        let current_index = to_visit.iter().position(|&p| p.eq(&current)).unwrap();
